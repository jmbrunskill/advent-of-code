...
func processInput(f io.Reader, down, right int) string {
	...
	s11 := treesOnSlope(1, 1, treeline)
	s31 := treesOnSlope(1, 3, treeline)
	s51 := treesOnSlope(1, 5, treeline)
	s71 := treesOnSlope(1, 7, treeline)
	s12 := treesOnSlope(2, 1, treeline)

	result = s11 * s31 * s51 * s71 * s12
