...
func countShinyGoldBags(bagName string, bagRules map[string]Bag, bagContainsCount map[string]int) int {
	count := 0
	...
	if bag, ok := bagRules[bagName]; ok {
		for i, name := range bag.ContainedBagNames {
			if name == "shiny gold" {
				count += bag.ContainedBagCounts[i]
			} else {
				count += countShinyGoldBags(name, bagRules, bagContainsCount)
			}
		}
	}
...
func processInput(f io.Reader) string {
	...
	for k, _ := range bagRules {
		count := countShinyGoldBags(k, bagRules, bagContainsCount)
		// fmt.Println(k, count)
		if count > 0 {
			result++
		}
