...
func processInput(f io.Reader) string {
	...
	for _, line := range lines {
		...
		case "Guard":
			...
			if currentGuard != 0 && !currentGuardAwake {
				...
				sleepTime := sleepers[currentGuard].addSleep(lastEventMin, 60)
				if sleepTime > maxSleep {
					maxSleeper = currentGuard
					maxSleep = sleepTime
				}
			}
		...
		case "wakes":
			...
			if currentGuard != 0 && !currentGuardAwake {
				...
				sleepTime := sleepers[currentGuard].addSleep(lastEventMin, thisMinute)
				if sleepTime > maxSleep {
					maxSleeper = currentGuard
					maxSleep = sleepTime
				}
	...
	sleepiestMinute, _ := sleepers[maxSleeper].sleepiestMinute()

	return fmt.Sprintf("%v", maxSleeper*sleepiestMinute)
}
