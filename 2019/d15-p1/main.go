...
func runIntCodeChans(code []int, output chan int, input chan int, exitcode chan int, errors chan error) {
...
func processInput(f io.Reader) string {
	...
	var r robotState
	r.robotMap = make(map[xy]string)
	r.robotLocation = xy{0, 0}
	//Start Robot facing north
	...
	go runIntCodeChans(intProgramSlice, out, in, exitcode, errors)
	...
	for {
		...
		case inst, ok := <-out:
			if !ok {
				fmt.Println("intcode output error", inst, ok)
				return "NOTOK"
			}
			fmt.Printf("Output %d\n", inst)
			switch inst {
...
func (r *robotState) wall() {
	//First we need to record where the wall is...
	//When you hit a wall, I always find it's best to turn left :)

	//north (1), south (2), west (3), and east (4)
	switch r.robotDirection {
	case 1:
		r.robotMap[xy{r.robotLocation.x, r.robotLocation.y + 1}] = "#"
		//North, so turn west
		r.robotDirection = 3
	case 2:
		r.robotMap[xy{r.robotLocation.x, r.robotLocation.y - 1}] = "#"
		...
		r.robotDirection = 4
	case 3:
		r.robotMap[xy{r.robotLocation.x - 1, r.robotLocation.y}] = "#"
		//West, so turn south
		r.robotDirection = 2
	case 4:
		r.robotMap[xy{r.robotLocation.x + 1, r.robotLocation.y}] = "#"
	...
	if r.robotLocation.x-1 < r.minX {
		r.minX = r.robotLocation.x - 1
	}
	if r.robotLocation.x+1 > r.maxX {
		r.maxX = r.robotLocation.x + 1
	}
	if r.robotLocation.y-1 < r.minY {
		r.minY = r.robotLocation.y - 1
	}
	if r.robotLocation.y+1 < r.minY {
		r.minY = r.robotLocation.y + 1
	}
...
func (r *robotState) move() {
	//Update robotLocation
	switch r.robotDirection {
	case 1:
		r.robotLocation = xy{r.robotLocation.x, r.robotLocation.y + 1}
	case 2:
		r.robotLocation = xy{r.robotLocation.x, r.robotLocation.y - 1}
	case 3:
		r.robotLocation = xy{r.robotLocation.x - 1, r.robotLocation.y}
	case 4:
		r.robotLocation = xy{r.robotLocation.x + 1, r.robotLocation.y}
	...
	if r.robotLocation.x < r.minX {
		r.minX = r.robotLocation.x
	}
	if r.robotLocation.x > r.maxX {
		r.maxX = r.robotLocation.x
	}
	if r.robotLocation.y < r.minY {
		r.minY = r.robotLocation.y
	}
	if r.robotLocation.y < r.minY {
		r.minY = r.robotLocation.y
	}
}
func (r *robotState) foundOxygen() {
	r.Print()
	panic("FOUND IT WHAT NOW!")
}

func (r *robotState) Print() {

	for y := max(r.maxY, 2); y >= min(r.minY, -2); y-- {
		if y < 0 {
			fmt.Printf("%d|", y)
		} else {
			fmt.Printf("+%d|", y)
		...
		for x := min(r.minX, -2); x <= max(r.maxX, 2); x++ {
			if r.robotLocation.x == x && r.robotLocation.y == y {
				switch r.robotDirection {
				case 1:
					//North
					fmt.Printf("^")
				...
				if (r.robotMap[xy{x, y}] != "") {
					fmt.Printf(r.robotMap[xy{x, y}])
				} else {
					fmt.Print(".")
				}
			}
