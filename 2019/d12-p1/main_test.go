...
func TestCalculateTotalEnergy(t *testing.T) {
	tt := []struct {
		...
		{[]xyz{xyz{-1, 0, 2}, xyz{2, -10, -7}, xyz{4, -8, 8}, xyz{3, 5, -1}}, 10, 179},
		{[]xyz{xyz{-8, -10, 0}, xyz{5, 5, 10}, xyz{2, -7, 3}, xyz{9, -8, -3}}, 100, 1940},
	}

	for _, tc := range tt {
		t.Run(fmt.Sprintf("%d -> %v", tc.moons, tc.expected), func(t *testing.T) {
			result := calculateTotalEnergy(tc.moons, tc.steps)
